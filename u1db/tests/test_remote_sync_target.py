...
class TestRemoteSyncTargets(tests.TestCaseWithServer):
    ...
    def test_sync_exchange_send_failure_and_retry_scenario(self):
        ...
        def bomb_put_doc_if_newer(doc, replica_uid=None, replica_gen=None):
            if doc.doc_id in trigger_ids:
                raise Exception
            return _put_doc_if_newer(doc, replica_uid=replica_uid,
        ...
        # bounced back to us
        self.assertEqual([('doc-here', 'replica:1', '{"value": "here"}')],
                         other_docs)
